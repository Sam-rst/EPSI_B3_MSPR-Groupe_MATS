...
from ui.styles import (
    MAIN_BG_COLOR, TITLE_FONT, BUTTON_FONT, LABEL_FONT_ITALIC,
    ACCENT_COLOR, LIGHT_BG_COLOR, BORDER_COLOR, BORDER_THICKNESS,
...
class UploadPanel:
    ...
    def _on_drop(self, event):
        ...
        self.drop_frame.after(1000, lambda: self.drop_frame.config(highlightbackground=ACCENT_COLOR))
    ...
    def _remove_file(self, index):
        if 0 <= index < len(self.selected_files):
            ...
            # Mettre à jour le statut
            if self.status_callback:
                self.status_callback(f"Fichier retiré. {len(self.selected_files)} fichier(s) restant(s)")