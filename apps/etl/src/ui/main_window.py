...
class MainWindow:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            header_frame, text="Analyze IT", font=TITLE_FONT, bg=MAIN_BG_COLOR
        )
        ...
        if self.user:
            ...
            logout_button = tk.Button(
                user_frame, text="Déconnexion", font=ITEM_FONT, command=self.logout
            )
        ...
        self.status_label = tk.Label(
            self.main_frame, text="", font=ITEM_FONT, bg=MAIN_BG_COLOR
        )
        ...
        self.progress_bar = ttk.Progressbar(
            self.main_frame, orient="horizontal", mode="determinate", length=400
        )
    ...
    def change_output_dir(self):
        ...
        new_dir = filedialog.askdirectory(
            title="Sélectionner le répertoire de sortie", initialdir=self.output_dir
        )
    ...
    def add_files(self):
        ...
        file_paths = filedialog.askopenfilenames(
            title="Sélectionner des fichiers CSV", filetypes=filetypes
        )
    ...
    def on_drop(self, event):
        ...
        if csv_files:
            self.file_list_panel.add_files(csv_files)
            self.update_status(
                f"{len(csv_files)} fichier(s) CSV ajouté(s) par glisser-déposer"
            )
    ...
    def run_pipeline(self):
        ...
        if selected_count > 30:
            messagebox.showerror(
                "Erreur", "Le traitement est limité à 30 fichiers maximum."
            )
            return
        ...
        if selected_count > 10:
            messagebox.showwarning(
                "Avertissement",
                "Le traitement de plus de 10 fichiers peut prendre du temps.",
            )
    ...
    def connect_to_database(self):
        ...
        if not self.pipeline.transformed_datasets:
            messagebox.showwarning(
                "Attention", "Veuillez d'abord traiter des fichiers CSV"
            )
            return
        ...
        tk.Entry(main_frame, textvariable=host_var, width=30).grid(
            row=0, column=1, padx=10, pady=5
        )

        tk.Label(main_frame, text="Database:").grid(
            row=1, column=0, sticky="e", padx=10, pady=5
        )
        tk.Entry(main_frame, textvariable=db_var, width=30).grid(
            row=1, column=1, padx=10, pady=5
        )

        tk.Label(main_frame, text="User:").grid(
            row=2, column=0, sticky="e", padx=10, pady=5
        )
        tk.Entry(main_frame, textvariable=user_var, width=30).grid(
        ...
        tk.Entry(main_frame, textvariable=password_var, width=30, show="*").grid(
        ...
        tk.Entry(main_frame, textvariable=port_var, width=30).grid(
        ...
        def load_data():
            try:
                ...
                if not self.pipeline.transformed_datasets:
                    messagebox.showwarning(
                        "Attention", "Aucun fichier transformé disponible"
                    )
                    return
                ...
                transformed_files = [
                    f"{name}_cleaned.csv"
                    for name in self.pipeline.transformed_datasets.keys()
                ]
                ...
                if db.execute_etl_process(
                    ...
                    messagebox.showinfo(
                        "Succès", "Données chargées avec succès dans PostgreSQL"
                    )
        ...
        test_button = tk.Button(
            button_frame, text="Tester connexion", command=test_connection
        )
        test_button.pack(side=tk.LEFT, padx=10)

        load_button = tk.Button(
            button_frame, text="Charger dans PostgreSQL", command=load_data
        )
