        ...
        for col in df.columns:
            if pd.api.types.is_datetime64_dtype(df[col]):
                df.loc[:, col] = df[col].fillna(
                    pd.NaT
                )  # Laisse les dates manquantes comme NaT
            elif pd.api.types.is_numeric_dtype(df[col]):
                df.loc[:, col] = df[col].fillna(0)  # Remplace les NaN numériques par 0
            else:
                df.loc[:, col] = df[col].fillna(
                    "Unknown"
                )  # Remplace les NaN texte par "Unknown"
        return df
    ...
    def standardize_dates(self, df):
        ...
        if "ReportDate" in df.columns:
            ...
            df["ReportDate"] = pd.to_datetime(
                df["ReportDate"], format="%Y-%m-%d", errors="coerce"
            )
        return df
    ...
    def harmonize_countries(self, df):
        ...
        if "Country" in df.columns:
            ...
            df["Country"] = df["Country"].apply(
                lambda x: (
                    reverse_map.get(x.strip().lower(), x) if isinstance(x, str) else x
                )
            )
        return df
