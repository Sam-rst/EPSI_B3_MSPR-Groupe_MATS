...
class AdminDashboardWindow(tk.Frame):
    def __init__(self, master, language_manager):
        ...
        self.available_regions = ["global", "france", "états-unis", "allemagne", "royaume-uni", "espagne", "italie"]
        
        # Language selector
        self.language_frame = tk.Frame(self)
        self.language_frame.pack(anchor="ne", padx=10, pady=5)
        
        self.language_label = tk.Label(self.language_frame, text=self.language_manager.get_text("language"))
        ...
        current_language_index = language_values.index(self.language_manager.current_language)
    ...
    def _create_widgets(self):
        # Titre
        self.title_label = tk.Label(self, text=self.language_manager.get_text("dashboard_title"), font=("Arial", 16, "bold"))
        ...
        self.left_frame = tk.LabelFrame(self.main_container, text=self.language_manager.get_text("add_user_frame"))
        ...
        # Username
        self.username_label = tk.Label(self.form_frame, text=self.language_manager.get_text("username"))
        ...
        # Password
        self.password_label = tk.Label(self.form_frame, text=self.language_manager.get_text("password"))
        ...
        # Role (dropdown)
        self.role_label = tk.Label(self.form_frame, text=self.language_manager.get_text("role"))
        self.role_label.grid(row=2, column=0, sticky="w", padx=5, pady=5)
        self.role_var = tk.StringVar()
        self.role_dropdown = ttk.Combobox(self.form_frame, textvariable=self.role_var, 
                                          values=self.available_roles, width=22, state="readonly")
        self.role_dropdown.grid(row=2, column=1, padx=5, pady=5)
        self.role_dropdown.current(1)  # Default to "user"
        
        # Region (dropdown)
        self.region_label = tk.Label(self.form_frame, text=self.language_manager.get_text("region"))
        self.region_label.grid(row=3, column=0, sticky="w", padx=5, pady=5)
        self.region_var = tk.StringVar()
        self.region_dropdown = ttk.Combobox(self.form_frame, textvariable=self.region_var, 
                                           values=self.available_regions, width=22, state="readonly")
        self.region_dropdown.grid(row=3, column=1, padx=5, pady=5)
        self.region_dropdown.current(0)  # Default to "global"
        
        # Add button
        self.add_button = tk.Button(self.form_frame, text=self.language_manager.get_text("add_button"), 
                                   command=self._add_user)
        self.add_button.grid(row=4, column=0, columnspan=2, pady=10)
        
        # Right side - User list
        self.right_frame = tk.LabelFrame(self.main_container, text=self.language_manager.get_text("user_list_frame"))
        self.right_frame.pack(side="right", fill="both", expand=True, padx=5, pady=5)
        
        # Treeview for users
        columns = ("login", "role", "region")
        self.user_tree = ttk.Treeview(self.right_frame, columns=columns, show="headings")
        ...
        self.user_tree.heading("region", text=self.language_manager.get_text("region_col"))
        ...
        # Scrollbar
        scrollbar = ttk.Scrollbar(self.right_frame, orient="vertical", command=self.user_tree.yview)
        ...
        # Delete button
        self.delete_button = tk.Button(self.button_frame, text=self.language_manager.get_text("delete_button"), 
                                     command=self._delete_user)
        self.delete_button.pack(side="left", padx=5)
        
        # Refresh button
        self.refresh_button = tk.Button(self.button_frame, text=self.language_manager.get_text("refresh_button"), 
    ...
    def _update_text(self):
        ...
        self.refresh_button.config(text=self.language_manager.get_text("refresh_button"))
