...
try:
    ...
    has_dnd = False
    print("tkinterdnd2 n'est pas installé. Le glisser-déposer ne sera pas disponible dans l'ETL.")
...
class LauncherApp:
    ...
    def show_login_screen(self):
        ...
        self.main_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER, relwidth=0.8, relheight=0.8)
    ...
    def _authenticate(self, username, password):
        """Vérifie si les identifiants sont valides en utilisant l'API"""
        user_data = self.api.authenticate(username, password)
        if user_data:
            # Stocker les infos utilisateur pour une utilisation ultérieure
            self.current_user = {
                "login": username,
                "role": user_data["role"],
    ...
    def show_app_selection_screen(self):
        ...
        user_label = tk.Label(
            self.main_frame,
            text=f"Connecté en tant que : {self.current_user['login']} ({self.current_user['role']})",
            font=LABEL_FONT,
            bg=MAIN_BG_COLOR
        )
    ...
    def launch_etl_app(self):
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location("main_window", os.path.join(CURRENT_DIR, "ui", "main_window.py"))
            ...
            print(f"Application ETL lancée pour l'utilisateur {self.current_user['login']}")
    ...
    def launch_admin_dashboard(self):
        ...
        try:
            ...
            print(f"Dashboard administrateur lancé pour l'utilisateur {self.current_user['login']}")
            
        except Exception as e:
            messagebox.showerror("Erreur", f"Impossible de lancer le Dashboard Admin: {str(e)}")
