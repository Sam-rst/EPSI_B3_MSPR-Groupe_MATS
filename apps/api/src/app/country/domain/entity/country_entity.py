...
class CountryEntity(BaseEntity):
    ...
    def print(self) -> str:
        return f"Country n°{self.id}: {self.name} ({self.iso2}, {self.iso3}, {self.code3}) a {self.population} habitants"
    ...
    @code3.setter
    def code3(self, value: str):
        if not value:
            raise ValueError("Le code3 ne peut pas être vide.")
        self._code3 = value
        self.update("system")
